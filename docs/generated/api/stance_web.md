...
### `serve_dashboard(host: str = 127.0.0.1, port: int = 8080, storage: StorageBackend | None, open_browser: bool = True) -> None`

Start the Stance dashboard server.  This is a convenience function that creates a StanceServer and starts it. The function blocks until the server is stopped (via Ctrl+C).
...
- `open_browser` (`bool`) - default: `True` - Whether to open browser automatically
